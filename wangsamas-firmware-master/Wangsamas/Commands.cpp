...
void Commands::printCurrentPosition(FSTRINGPARAM(s)) {
    ...
    Com::printFLN(Com::tSpaceEColon, Printer::currentPositionSteps[E_AXIS] * Printer::invaxisStepsPerUnit[E_AXIS] * (Printer::unitIsInches ? 0.03937 : 1), 4);
...
void Commands::processArc(GCode *com) {
    ...
    float target[E_AXIS_ARRAY] = {Printer::realXPosition(),Printer::realYPosition(),Printer::realZPosition(),Printer::destinationSteps[E_AXIS]*Printer::invaxisStepsPerUnit[E_AXIS]};
    float r;
    if (com->hasR()) {
        /*
...
//    Com::printFLN(PSTR("com->G :"),(int)com->G);
//    Com::printFLN(PSTR("com->X :"),com->X);
//    Com::printFLN(PSTR("com->Y :"),com->Y);
//    Com::printFLN(PSTR("com->Z :"),com->Z);
    ...
    switch(com->G) {
...
#else
                        Printer::currentPositionSteps[Z_AXIS] = sum * Printer::axisStepsPerUnit[Z_AXIS];
		...
		case 5:				// Kusuma SCARA, Rotate Arm Sample G5 X10 Y20 means rotate shoulder 10 degree elbow 20 degree
		{
			...
			int32_t rotY = com->Y * Printer::axisStepsPerUnit[X_AXIS];
			PrintLine::rotateInSteps(rotX, rotY, Printer::maxFeedrate[Y_AXIS]/10, true, true);
			Printer::updateCurrentPosition(true);
			Com::printF(PSTR(" Shoulder Angle	: "), Printer::currentNonlinearPositionSteps[X_AXIS] * Printer::invaxisStepsPerUnit[X_AXIS]);			
			Com::printFLN(PSTR("	Elbow Angle		: "), Printer::currentNonlinearPositionSteps[Y_AXIS] * Printer::invaxisStepsPerUnit[Y_AXIS]);			
			Com::printF(PSTR(" X position		: "), Printer::currentPosition[X_AXIS]);			
			Com::printFLN(PSTR("	Y position		: "), Printer::currentPosition[Y_AXIS]);	
			com->X = com->Y = 0;
		}
		break;
/* SCARA CALIBRATION 
...
*/
		case 50:
		{
			...
			{
				if(com->P == 1){
					Printer::deltaAPosXSteps = Printer::currentNonlinearPositionSteps[X_AXIS];
					Printer::deltaAPosYSteps = Printer::currentNonlinearPositionSteps[Y_AXIS];
					Com::printFLN(PSTR(" Point 1 Shoulder Angle	: "), Printer::deltaAPosXSteps * Printer::invaxisStepsPerUnit[X_AXIS]);		
					Com::printFLN(PSTR(" Point 1 Elbow Angle		: "), Printer::deltaAPosYSteps * Printer::invaxisStepsPerUnit[Y_AXIS]);				
				}
				else if (com->P == 2){
					Printer::deltaBPosXSteps = Printer::currentNonlinearPositionSteps[X_AXIS];
					Printer::deltaBPosYSteps = Printer::currentNonlinearPositionSteps[Y_AXIS];					
					Com::printFLN(PSTR(" Point 2 Shoulder Angle	: "), Printer::deltaBPosXSteps * Printer::invaxisStepsPerUnit[X_AXIS]);		
					Com::printFLN(PSTR(" Point 2 Elbow Angle		: "), Printer::deltaBPosYSteps * Printer::invaxisStepsPerUnit[Y_AXIS]);				
				}
				else if (com->P == 3){
					Printer::deltaCPosXSteps = Printer::currentNonlinearPositionSteps[X_AXIS];
					Printer::deltaCPosYSteps = Printer::currentNonlinearPositionSteps[Y_AXIS];					
					Com::printFLN(PSTR(" Point 3 Shoulder Angle	: "), Printer::deltaCPosXSteps * Printer::invaxisStepsPerUnit[X_AXIS]);		
					Com::printFLN(PSTR(" Point 3 Elbow Angle		: "), Printer::deltaCPosYSteps * Printer::invaxisStepsPerUnit[Y_AXIS]);				
				}
				else if (com->P == 4){
					Printer::deltaDPosXSteps = Printer::currentNonlinearPositionSteps[X_AXIS];
					Printer::deltaDPosYSteps = Printer::currentNonlinearPositionSteps[Y_AXIS];					
					Com::printFLN(PSTR(" Point 4 Shoulder Angle	: "), Printer::deltaDPosXSteps * Printer::invaxisStepsPerUnit[X_AXIS]);		
					Com::printFLN(PSTR(" Point 4 Elbow Angle		: "), Printer::deltaDPosYSteps * Printer::invaxisStepsPerUnit[Y_AXIS]);				
				}
				else if(com->P == 5){ // check forearm calibration
				float a = sqrt(RMath::sqr(Printer::deltaBPosYSteps-Printer::deltaAPosYSteps)) * Printer::invaxisStepsPerUnit[Y_AXIS] / 57.29577951472 / 2;
				a = Printer::ForearmLength * 2 * sin(a);
				Com::printFLN(PSTR(" Distance check p1 p2 for forearm : "), a);     
				}
				else
				{
...
//					Printer::sqFarm = Printer::ForearmLength*Printer::ForearmLength;
					...
					Com::printFLN(PSTR(" Elbow Step per Unit	: "), Printer::axisStepsPerUnit[Y_AXIS]);
					Com::printFLN(PSTR(" G50 S11 if you want to save result"));					
				}
				else if(com->S == 11){
					EEPROM::storeScaraDataIntoEEPROM();
					Printer::updateDerivedParameter();
					Com::printFLN(PSTR(" Calibrating ForeArm finished"));									
					Com::printFLN(PSTR(" ForeArm Length			: "), EEPROM::forearmLength());	
					...
					Com::printFLN(PSTR(" Shoulder Axis Step per mm	: "), Printer::radius0);
					Com::printFLN(PSTR(" R1	: "), Printer::deltaAPosX);
					Com::printFLN(PSTR(" Enter G50 X Y Z S22 to continue step 2"));									
				}
				else if(com->S == 22){
					Printer::deltaAPosY = R; 
					Printer::radius0 = sqrt(RMath::sqr((Printer::deltaDPosXSteps-Printer::deltaCPosXSteps) / d));
					Com::printFLN(PSTR(" Step 2 calibrating Arm"));									
					Com::printFLN(PSTR(" Shoulder Axis Step per mm	: "), Printer::radius0);
					Com::printFLN(PSTR(" Compare Shoulder Axis Step per mm step 1 & step 2. It should not different too much"));									
					Com::printFLN(PSTR(" R2	: "), Printer::deltaAPosY);			
					Com::printFLN(PSTR(" Enter G50 S23 to continue step 3"));									
				}
				else if(com->S == 23){
					a = Printer::ForearmLength + Printer::deltaAPosX;
					b = Printer::ForearmLength - Printer::deltaAPosX;
					float beta = (Printer::deltaCPosYSteps - Printer::deltaAPosYSteps)* Printer::invaxisStepsPerUnit[Y_AXIS]/ 57.29577951472;
					float c = (cos(beta) - 1)/ sin(beta);
					float d = ((cos(beta) - 1) * Printer::ForearmLength * Printer::ForearmLength + Printer::deltaAPosY * Printer::deltaAPosY - Printer::deltaAPosX * Printer::deltaAPosX * cos(beta)) / sin(beta);
					float e = c * c + 1;
					float f = 2 * c * d - a * a - b * b;
					float g = d * d + a * a * b * b;
					Printer::deltaBPosX = sqrt((-f + sqrt(f*f-4*e*g))/(2*e));
					Printer::deltaCPosX = sqrt((-f - sqrt(f*f-4*e*g))/(2*e));
					Printer::deltaBPosY = acos((Printer::deltaBPosX*Printer::deltaBPosX + Printer::ForearmLength*Printer::ForearmLength - Printer::deltaAPosX*Printer::deltaAPosX)/(2*Printer::deltaBPosX*Printer::ForearmLength))* 57.29577951472;
					Printer::deltaCPosY = acos((Printer::deltaCPosX*Printer::deltaCPosX + Printer::ForearmLength*Printer::ForearmLength - Printer::deltaAPosX*Printer::deltaAPosX)/(2*Printer::deltaCPosX*Printer::ForearmLength))* 57.29577951472;
					Printer::deltaBPosY = Printer::ElbowMaxAngle + 180 - Printer::deltaAPosYSteps * Printer::invaxisStepsPerUnit[Y_AXIS] - Printer::deltaBPosY;
					Printer::deltaCPosY = Printer::ElbowMaxAngle + 180 - Printer::deltaAPosYSteps * Printer::invaxisStepsPerUnit[Y_AXIS] - Printer::deltaCPosY;
					Printer::axisStepsPerUnit[X_AXIS] = Printer::radius0;
					Printer::invaxisStepsPerUnit[X_AXIS] = 1.0f / Printer::axisStepsPerUnit[X_AXIS];
					Com::printFLN(PSTR(" Step 3 calibrating Arm"));									
					Com::printFLN(PSTR(" Shoulder Steps per Unit		: "), Printer::axisStepsPerUnit[X_AXIS]);
					Com::printFLN(PSTR(" Shoulder Inverse Steps per Unit: "), Printer::invaxisStepsPerUnit[X_AXIS]);
					Com::printFLN(PSTR(" Arm Length			Opt 1: "), Printer::deltaBPosX);			
					Com::printFLN(PSTR(" Elbow Max Angle	Opt 1: "), Printer::deltaBPosY);					
					Com::printFLN(PSTR(" Enter g50 s231 to save Opt 1"));
					Com::printFLN(PSTR(" Arm Length			Opt 2: "), Printer::deltaCPosX);			
					Com::printFLN(PSTR(" Elbow Max Angle	Opt 2: "), Printer::deltaCPosY);					
					Com::printFLN(PSTR(" Enter g50 s232 to save Opt 2"));
				}
				else if(com->S == 231){
...
//					Printer::sqArm = Printer::ArmLength*Printer::ArmLength;
					...
					Com::printFLN(PSTR(" Elbow Max Angle	: "), EEPROM::ElbowMaxAngle());	
					Com::printFLN(PSTR(" Shoulder Steps per Unit	: "), Printer::axisStepsPerUnit[X_AXIS]);
					Com::printFLN(PSTR(" Saved to EEPROM"));									
				}
				else if(com->S == 232){
...
/*		case 51:			// Kusuma SCARA, S1 set bed center, X1 set shoulder max angle, Y1 set shoulder max angle
		{
			if(com->hasS() && com->S == 1)
			{
				...
				Printer::ElbowBedCenterAngle = Printer::currentNonlinearPositionSteps[Y_AXIS]* Printer::invaxisStepsPerUnit[Y_AXIS];
				HAL::eprSetFloat(EPR_SHOULDER_BED_CENTER_ANGLE,Printer::ShoulderBedCenterAngle);
				HAL::eprSetFloat(EPR_ELBOW_BED_CENTER_ANGLE,Printer::ElbowBedCenterAngle);
				Printer::updateDerivedParameter();
				transformScaraStepsToCartesianSteps(Printer::currentNonlinearPositionSteps,Printer::currentPositionSteps);
				Printer::updateCurrentPosition(true);
				Com::printFLN(Com::tShoulderBedCenterAngle, Printer::ShoulderBedCenterAngle);
				Com::printFLN(Com::tElbowBedCenterAngle, Printer::ElbowBedCenterAngle);
				Commands::printCurrentPosition(PSTR("UI_ACTION_SET_BED_CENTER "));
			}
			if(com->hasX() && com->X == 1)
			{
				Printer::updateCurrentPosition();
#if MAX_HARDWARE_ENDSTOP_X
				Printer::ShoulderMaxAngle -= Printer::currentNonlinearPositionSteps[X_AXIS]*Printer::invaxisStepsPerUnit[X_AXIS];
				HAL::eprSetFloat(EPR_SHOULDER_MAX_ANGLE,Printer::ShoulderMaxAngle);
				Printer::currentNonlinearPositionSteps[X_AXIS] = 0;
#else
			Printer::ShoulderMaxAngle = Printer::currentNonlinearPositionSteps[X_AXIS]*Printer::invaxisStepsPerUnit[X_AXIS];
			HAL::eprSetFloat(EPR_SHOULDER_MAX_ANGLE,Printer::ShoulderMaxAngle);
#endif
            transformScaraStepsToCartesianSteps(Printer::currentNonlinearPositionSteps, Printer::currentPositionSteps);
            Printer::updateCurrentPosition(true);
            Com::printFLN(Com::tShoulderMaxAngle, Printer::ShoulderMaxAngle);
			Printer::updateDerivedParameter();
            Commands::printCurrentPosition(PSTR("UI_ACTION_SET_SHOULDER_ORIGIN "));
			}
			if(com->hasY() && com->Y == 1)
			{
            Printer::updateCurrentPosition();
#if MAX_HARDWARE_ENDSTOP_Y
			Printer::ElbowMaxAngle -= Printer::currentNonlinearPositionSteps[Y_AXIS]*Printer::invaxisStepsPerUnit[Y_AXIS];
            HAL::eprSetFloat(EPR_ELBOW_MAX_ANGLE,Printer::ElbowMaxAngle);
            Printer::currentNonlinearPositionSteps[Y_AXIS] = 0;
#else
			Printer::ElbowMaxAngle = Printer::currentNonlinearPositionSteps[Y_AXIS]*Printer::invaxisStepsPerUnit[Y_AXIS];
            HAL::eprSetFloat(EPR_ELBOW_MAX_ANGLE,Printer::ElbowMaxAngle);
#endif	
            transformScaraStepsToCartesianSteps(Printer::currentNonlinearPositionSteps, Printer::currentPositionSteps);
            Printer::updateCurrentPosition(true);
            Com::printFLN(Com::tElbowMaxAngle, Printer::ElbowMaxAngle);
            Printer::updateDerivedParameter();
            Commands::printCurrentPosition(PSTR("UI_ACTION_SET_ELBOW_ORIGIN "));
			}
		}
		break;
		case 52:			// Kusuma SCARA, Calibrate Arm & Forearm length, G X Y S  
		{
			if(com->hasS() && com->S == 1)
			{
			Printer::deltaAPosXSteps = Printer::currentNonlinearPositionSteps[X_AXIS];
			Printer::deltaAPosYSteps = Printer::currentNonlinearPositionSteps[Y_AXIS];
			Printer::deltaAPosX = com->X;
			Printer::deltaAPosY = com->Y;
			Com::printFLN(PSTR(" Point 1 Shoulder Angle	: "), Printer::deltaAPosXSteps * Printer::invaxisStepsPerUnit[X_AXIS]);			
			Com::printFLN(PSTR(" Point 1 Elbow Angle		: "), Printer::deltaAPosYSteps * Printer::invaxisStepsPerUnit[Y_AXIS]);			
			Com::printFLN(PSTR(" Point 1 X position		: "), Printer::deltaAPosX);			
			Com::printFLN(PSTR(" Point 1 Y position		: "), Printer::deltaAPosY);			
			com->X = com->Y = com->S = 0;
			break;
			}
			else if(com->hasS() && com->S == 2)
			{
			Printer::deltaBPosX = com->X;
			Printer::deltaBPosY = com->Y;
			Printer::deltaBPosXSteps = Printer::currentNonlinearPositionSteps[X_AXIS];
			Printer::deltaBPosYSteps = Printer::currentNonlinearPositionSteps[Y_AXIS];
			Com::printFLN(PSTR(" Point 2 Shoulder Angle	: "), Printer::deltaBPosXSteps * Printer::invaxisStepsPerUnit[X_AXIS]);			
			Com::printFLN(PSTR(" Point 2 Elbow Angle		: "), Printer::deltaBPosYSteps * Printer::invaxisStepsPerUnit[Y_AXIS]);			
			Com::printFLN(PSTR(" Point 2 X position		: "), Printer::deltaBPosX);			
			Com::printFLN(PSTR(" Point 2 Y position		: "), Printer::deltaBPosY);			
			com->X = com->Y = com->S = 0;
			break;
			}
			else if(com->hasS() && com->S == 3)
			{
			Printer::deltaCPosX = com->X;
			Printer::deltaCPosY = com->Y;
			Printer::deltaCPosXSteps = Printer::currentNonlinearPositionSteps[X_AXIS];
			Printer::deltaCPosYSteps = Printer::currentNonlinearPositionSteps[Y_AXIS];
			Com::printFLN(PSTR(" Point 3 Shoulder Angle	: "), Printer::deltaCPosXSteps * Printer::invaxisStepsPerUnit[X_AXIS]);			
			Com::printFLN(PSTR(" Point 3 Elbow Angle		: "), Printer::deltaCPosYSteps * Printer::invaxisStepsPerUnit[Y_AXIS]);			
			Com::printFLN(PSTR(" Point 3 X position		: "), Printer::deltaCPosX);			
			Com::printFLN(PSTR(" Point 3 Y position		: "), Printer::deltaCPosY);			
			com->X = com->Y = com->S = 0;
			break;
			}
			else if(com->hasS() && com->S == 4)
			{
			float a1, a2, a3, b1, b2, b3, c1, c2, c3, d1, d2, d3, e1, e2, e3, f1, f2, f3, g1, g2, g3, h1, h2, i1, i2, j1, j2;
			a1 = Printer::deltaAPosXSteps * Printer::invaxisStepsPerUnit[X_AXIS] / 57.29577951472;
			a2 = Printer::deltaBPosXSteps * Printer::invaxisStepsPerUnit[X_AXIS] / 57.29577951472;
			a3 = Printer::deltaCPosXSteps * Printer::invaxisStepsPerUnit[X_AXIS] / 57.29577951472;
			b1 = Printer::deltaAPosYSteps * Printer::invaxisStepsPerUnit[Y_AXIS] / 57.29577951472;
			b2 = Printer::deltaBPosYSteps * Printer::invaxisStepsPerUnit[Y_AXIS] / 57.29577951472;
			b3 = Printer::deltaCPosYSteps * Printer::invaxisStepsPerUnit[Y_AXIS] / 57.29577951472;
			c1 = a1+b1;
			c2 = a2+b2;
			c3 = a3+b3;
			d1 = RMath::sqr(Printer::deltaAPosX - Printer::deltaBPosX) + RMath::sqr(Printer::deltaAPosY - Printer::deltaBPosY);
			d2 = RMath::sqr(Printer::deltaBPosX - Printer::deltaCPosX) + RMath::sqr(Printer::deltaBPosY - Printer::deltaCPosY);
			d3 = RMath::sqr(Printer::deltaAPosX - Printer::deltaCPosX) + RMath::sqr(Printer::deltaAPosY - Printer::deltaCPosY);
			e1 = (1 - cos(a1-a2))*2;
			e2 = (1 - cos(a2-a3))*2;
			e3 = (1 - cos(a1-a3))*2;
			f1 = (1 - cos(c1-c2))*2;
			f2 = (1 - cos(c2-c3))*2;
			f3 = (1 - cos(c1-c3))*2;
			g1 = (cos(b1) + cos(b2) - cos(a1-c2) - cos(a2-c1))*2;
			g2 = (cos(b2) + cos(b3) - cos(a2-c3) - cos(a3-c2))*2;
			g3 = (cos(b1) + cos(b3) - cos(a1-c3) - cos(a3-c1))*2;
			h1 = d1*e2 - d2*e1;
			h2 = d2*e3 - d3*e2;
			i1 = f1*e2 - f2*e1;
			i2 = f2*e3 - f3*e2;
			j1 = g1*e2 - g2*e1;
			j2 = g2*e3 - g3*e2;
			Printer::deltaDiagonalStepsSquaredB.f = sqrt((h1*j2-h2*j1)/(i1*j2-i2*j1));
			Printer::deltaDiagonalStepsSquaredA.f = (h1 - i1*Printer::deltaDiagonalStepsSquaredB.f*Printer::deltaDiagonalStepsSquaredB.f)/(Printer::deltaDiagonalStepsSquaredB.f*j1);
			Com::printFLN(PSTR(" Calculated ArmLength		: "), Printer::deltaDiagonalStepsSquaredA.f);			
			Com::printFLN(PSTR(" Calculated ForearmLength	: "), Printer::deltaDiagonalStepsSquaredB.f);			
			com->X = com->Y = com->S = 0;
			break;
			}
			else if(com->hasS() && com->S == 5)
			{
			Printer::ArmLength = Printer::deltaDiagonalStepsSquaredA.f;
			Printer::ForearmLength = Printer::deltaDiagonalStepsSquaredB.f;
			Printer::sqArm = Printer::ArmLength*Printer::ArmLength;
			Printer::sqFarm = Printer::ForearmLength*Printer::ForearmLength;
            HAL::eprSetFloat(EPR_ARM_LENGTH,Printer::ArmLength);			
			HAL::eprSetFloat(EPR_FOREARM_LENGTH,Printer::ForearmLength);			
			Com::printFLN(PSTR(" Stored to EEPROM ArmLength		: "), Printer::ArmLength);			
			Com::printFLN(PSTR(" Stored to EEPROM ForearmLength	: "), Printer::ForearmLength);			
			Printer::updateDerivedParameter();
			com->X = com->Y = com->S = 0;
			Printer::homeAxis(true,true,true);
			break;
		...
		break;
		case 55:			// Kusuma SCARA
		{
			if(com->hasP())
			{
				if(com->P == 1){
					Printer::deltaAPosXSteps = Printer::currentPosition[X_AXIS];
					Printer::deltaAPosYSteps = Printer::currentPosition[Y_AXIS];
					Com::printFLN(PSTR(" Point 1 X position	: "), Printer::deltaAPosXSteps);		
					Com::printFLN(PSTR(" Point 1 Y position	: "), Printer::deltaAPosYSteps);				
				}
				if(com->P == 2){
					Printer::deltaBPosXSteps = Printer::currentPosition[X_AXIS];
					Printer::deltaBPosYSteps = Printer::currentPosition[Y_AXIS];
					Com::printFLN(PSTR(" Point 2 X position	: "), Printer::deltaBPosXSteps);		
					Com::printFLN(PSTR(" Point 2 Y position	: "), Printer::deltaBPosYSteps);
					float d = sqrt(RMath::sqr(Printer::deltaBPosXSteps-Printer::deltaAPosXSteps)+RMath::sqr(Printer::deltaBPosYSteps-Printer::deltaAPosYSteps));
					Com::printFLN(PSTR(" Distance between point 1 & 2	: "), d);
				}	
			}	
		}
		break;*/
        case 58: { //G58 All Go To Endstops
                ...
                if(com->hasE())
                    Printer::currentPositionSteps[E_AXIS] = 0;
                if(homeAllAxis || !com->hasNoXYZ())
                    Printer::closeArm(homeAllAxis || com->hasX(),homeAllAxis || com->hasY(),homeAllAxis || com->hasZ());
#if defined(SUPPORT_LASER) && SUPPORT_LASER
			    LaserDriver::laserOn = oldLaser;
#endif
                Printer::updateCurrentPosition();
        ...
        case 92: { // G92
                ...
                if(com->hasE()) {
                    Printer::currentPositionSteps[E_AXIS] = Printer::convertToMM(com->E) * Printer::axisStepsPerUnit[E_AXIS];
                }
            }
            break;
#if DRIVE_SYSTEM == DELTA 
                    ...
                    if (com->hasX() || com->hasY() || com->hasZ())
                        ...
                        if (Printer::currentPosition[X_AXIS] == 0
                            ...
                            if(Printer::isLargeMachine()) {
                                ...
                                h = sqrt(h);
                                EEPROM::setRodRadius(h*Printer::invaxisStepsPerUnit[Z_AXIS]);
                            } else {
                                // calculate radius assuming we are at surface
                                // If Z is greater than 0 it will get calculated out for correct radius
                                // Use either A or B tower as they acnhor x cartesian axis and always have
...
#endif
                PrintLine::moveRelativeDistanceInSteps(0, 0, -5*Printer::axisStepsPerUnit[Z_AXIS], 0, Printer::homingFeedrate[Z_AXIS], true, true);
                Printer::homeAxis(true,true,true);
            }
            break;
#endif
		...
		case 133: { // G133 Measure steps to top
				...
				Printer::MeasureStepToEndstop(Printer::homingFeedrate[Z_AXIS]);
                ...
                PrintLine::moveRelativeDistanceInSteps(0, 0, Printer::axisStepsPerUnit[Z_AXIS] * -ENDSTOP_Z_BACK_MOVE, 0, Printer::homingFeedrate[Z_AXIS] / ENDSTOP_X_RETEST_REDUCTION_FACTOR, true, false);
                Printer::homeAxis(true,true,true);
            }
            break;
#endif
...
#define G134_PRECISION 0.05
                ...
                if(!bigError) {
		            for(int i = 0; i < NUM_EXTRUDER; i++) {
			            if(s >= 0 && i != s) continue;
				        extruder[i].zOffset = avg[i] * Printer::axisStepsPerUnit[Z_AXIS] / G134_REPETITIONS;
					}
#if EEPROM_MODE != 0
...
void Commands::processMCode(GCode *com) {
    switch( com->M ) {
        ...
        case 85: // M85
            ...
            break;
        case 92: // M92
            if(com->hasX()) Printer::axisStepsPerUnit[X_AXIS] = com->X;
            if(com->hasY()) Printer::axisStepsPerUnit[Y_AXIS] = com->Y;
            if(com->hasZ()) Printer::axisStepsPerUnit[Z_AXIS] = com->Z;
            Printer::updateDerivedParameter();
            if(com->hasE()) {
                Extruder::current->StepsPerUnit = com->E;
        ...
        case 280: // M280
			...
			if(com->hasS() && com->S > 0) {
				...
				PrintLine::moveRelativeDistanceInSteps(-Extruder::current->xOffset + static_cast<int32_t>(Printer::xLength*0.5*Printer::axisStepsPerUnit[X_AXIS]), 0, 0, 0, EXTRUDER_SWITCH_XY_SPEED, true, true);
