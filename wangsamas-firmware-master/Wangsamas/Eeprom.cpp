...
void EEPROM::restoreEEPROMSettingsFromConfiguration()
{
    ...
    Printer::axisStepsPerUnit[X_AXIS] = XAXIS_STEPS_PER_UNIT;
    Printer::axisStepsPerUnit[Y_AXIS] = YAXIS_STEPS_PER_UNIT;
    Printer::axisStepsPerUnit[Z_AXIS] = ZAXIS_STEPS_PER_UNIT;
    Printer::axisStepsPerUnit[E_AXIS] = 1;
...
#if NUM_EXTRUDER>0
    e = &extruder[0];
    e->StepsPerUnit = EXT0_STEPS_PER_UNIT;
...
#if NUM_EXTRUDER>1
    e = &extruder[1];
    e->StepsPerUnit = EXT1_STEPS_PER_UNIT;
...
#if NUM_EXTRUDER>2
    e = &extruder[2];
    e->StepsPerUnit = EXT2_STEPS_PER_UNIT;
...
#if NUM_EXTRUDER>3
    e = &extruder[3];
    e->StepsPerUnit = EXT3_STEPS_PER_UNIT;
...
#if NUM_EXTRUDER > 4
    e = &extruder[4];
    e->StepsPerUnit = EXT4_STEPS_PER_UNIT;
...
#if NUM_EXTRUDER > 5
    e = &extruder[5];
    e->StepsPerUnit = EXT5_STEPS_PER_UNIT;
...
void EEPROM::storeScaraDataIntoEEPROM(uint8_t corrupted) // Kusuma SCARA
{
	...
	HAL::eprSetFloat(EPR_SHOULDER_MIN_ANGLE,Printer::ShoulderMinAngle);
	HAL::eprSetFloat(EPR_ELBOW_MIN_ANGLE,Printer::ElbowMinAngle);
...
//#define EPR_ACCELERATION_TYPE 1
    HAL::eprSetFloat(EPR_XAXIS_STEPS_PER_UNIT,Printer::axisStepsPerUnit[X_AXIS]);
    HAL::eprSetFloat(EPR_YAXIS_STEPS_PER_UNIT,Printer::axisStepsPerUnit[Y_AXIS]);
    HAL::eprSetFloat(EPR_ZAXIS_STEPS_PER_UNIT,Printer::axisStepsPerUnit[Z_AXIS]);
    ...
    for(uint8_t i = 0; i < NUM_EXTRUDER; i++)
    {
        ...
        HAL::eprSetFloat(o+EPR_EXTRUDER_STEPS_PER_UNIT,e->StepsPerUnit);
...
void EEPROM::initalizeUncached()
{
    ...
    HAL::eprSetFloat(EPR_ELBOW_MIN_ANGLE,ELBOW_MIN_ANGLE);						// Kusuma SCARA
    HAL::eprSetFloat(EPR_SHOULDER_MAX_ANGLE,SHOULDER_MAX_ANGLE);			// Kusuma SCARA
    HAL::eprSetFloat(EPR_ELBOW_MAX_ANGLE,ELBOW_MAX_ANGLE);				// Kusuma SCARA
    HAL::eprSetFloat(EPR_SHOULDER_BED_CENTER_ANGLE,SHOULDER_BED_CENTER_ANGLE);	// Kusuma SCARA
    HAL::eprSetFloat(EPR_ELBOW_BED_CENTER_ANGLE,ELBOW_BED_CENTER_ANGLE);		// Kusuma SCARA
#endif																			// Kusuma SCARA
#if DRIVE_SYSTEM == DELTA
...
//#define EPR_ACCELERATION_TYPE 1
    Printer::axisStepsPerUnit[X_AXIS] = HAL::eprGetFloat(EPR_XAXIS_STEPS_PER_UNIT);
    Printer::axisStepsPerUnit[Y_AXIS] = HAL::eprGetFloat(EPR_YAXIS_STEPS_PER_UNIT);
    Printer::axisStepsPerUnit[Z_AXIS] = HAL::eprGetFloat(EPR_ZAXIS_STEPS_PER_UNIT);
    ...
    if(includeExtruder)
    {
        ...
        for(uint8_t i = 0; i < NUM_EXTRUDER; i++)
        {
            ...
            int o=i*EEPROM_EXTRUDER_LENGTH+EEPROM_EXTRUDER_OFFSET;
            Extruder *e = &extruder[i];
            e->StepsPerUnit = HAL::eprGetFloat(o+EPR_EXTRUDER_STEPS_PER_UNIT);
...
//#define EPR_ACCELERATION_TYPE 1
    ...
    writeFloat(EPR_ELBOW_MIN_ANGLE, Com::tElbowMinAngle);						// Kusuma SCARA
    writeFloat(EPR_SHOULDER_MAX_ANGLE, Com::tShoulderMaxAngle);			// Kusuma SCARA
    writeFloat(EPR_ELBOW_MAX_ANGLE, Com::tElbowMaxAngle);					// Kusuma SCARA
    writeFloat(EPR_SHOULDER_BED_CENTER_ANGLE, Com::tShoulderBedCenterAngle);	// Kusuma SCARA
    writeFloat(EPR_ELBOW_BED_CENTER_ANGLE, Com::tElbowBedCenterAngle);			// Kusuma SCARA
#endif																			// Kusuma SCARA
#if DRIVE_SYSTEM != DELTA
    writeFloat(EPR_XAXIS_STEPS_PER_UNIT, Com::tEPRXStepsPerUnit, 4);
    writeFloat(EPR_YAXIS_STEPS_PER_UNIT, Com::tEPRYStepsPerUnit, 4);
#endif
    writeFloat(EPR_ZAXIS_STEPS_PER_UNIT, Com::tEPRZStepsPerUnit, 4);
#if DRIVE_SYSTEM != DELTA
    ...
    {
        int o = i * EEPROM_EXTRUDER_LENGTH + EEPROM_EXTRUDER_OFFSET;
        writeFloat(o + EPR_EXTRUDER_STEPS_PER_UNIT, Com::tEPRStepsPerUnit);
