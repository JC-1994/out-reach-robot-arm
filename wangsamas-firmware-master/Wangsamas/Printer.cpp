...
float Printer::axisStepsPerUnit[E_AXIS_ARRAY] = {XAXIS_STEPS_PER_UNIT, YAXIS_STEPS_PER_UNIT, ZAXIS_STEPS_PER_UNIT, 1}; ///< Number of steps per unit needed.
float Printer::invaxisStepsPerUnit[E_AXIS_ARRAY]; ///< Inverse of axisStepsPerUnit for faster conversion
...
int32_t Printer::deltaDPosXSteps;	// Kusuma Scara
int32_t Printer::deltaDPosYSteps;	// Kusuma Scara
...
float Printer::ElbowMaxAngle;		// Kusuma Scara
float Printer::ShoulderBedCenterAngle;	// Kusuma Scara
float Printer::ElbowBedCenterAngle;		// Kusuma Scara
float Printer::BedCenterXpos;			// Kusuma Scara
float Printer::BedCenterYpos;			// Kusuma Scara
float Printer::ScaraEndstopPosX;		//Kusuma Scara
float Printer::ScaraEndstopPosY;		//Kusuma Scara						
float Printer::squaredscaraMinRadius;	//Kusuma Scara
float Printer::squaredscaraMaxRadius;	//Kusuma Scara
float Printer::deltaAPosX;
float Printer::deltaAPosY;
float Printer::deltaBPosX;
float Printer::deltaBPosY;
float Printer::deltaCPosX;
float Printer::deltaCPosY;	
#endif 									//Kusuma Scara
...
//    sqFarm = ForearmLength* ForearmLength;							// Kusuma SCARA
    ...
    xMinSteps = ShoulderMinAngle * axisStepsPerUnit[X_AXIS];
	yMinSteps = ElbowMinAngle * axisStepsPerUnit[Y_AXIS];
	zMinSteps = zMin * axisStepsPerUnit[Z_AXIS];	
#elif DRIVE_SYSTEM == DELTA		//Kusuma SCARA
    axisStepsPerUnit[X_AXIS] = axisStepsPerUnit[Y_AXIS] = axisStepsPerUnit[Z_AXIS];
    ...
    zMaxSteps = axisStepsPerUnit[Z_AXIS] * (zLength);
    towerAMinSteps = axisStepsPerUnit[A_TOWER] * xMin;
    towerBMinSteps = axisStepsPerUnit[B_TOWER] * yMin;
    towerCMinSteps = axisStepsPerUnit[C_TOWER] * zMin;
    ...
    deltaAPosXSteps = floor(radiusA * cos(EEPROM::deltaAlphaA() * M_PI / 180.0f) * axisStepsPerUnit[Z_AXIS] + 0.5f);
    deltaAPosYSteps = floor(radiusA * sin(EEPROM::deltaAlphaA() * M_PI / 180.0f) * axisStepsPerUnit[Z_AXIS] + 0.5f);
    deltaBPosXSteps = floor(radiusB * cos(EEPROM::deltaAlphaB() * M_PI / 180.0f) * axisStepsPerUnit[Z_AXIS] + 0.5f);
    deltaBPosYSteps = floor(radiusB * sin(EEPROM::deltaAlphaB() * M_PI / 180.0f) * axisStepsPerUnit[Z_AXIS] + 0.5f);
    deltaCPosXSteps = floor(radiusC * cos(EEPROM::deltaAlphaC() * M_PI / 180.0f) * axisStepsPerUnit[Z_AXIS] + 0.5f);
    deltaCPosYSteps = floor(radiusC * sin(EEPROM::deltaAlphaC() * M_PI / 180.0f) * axisStepsPerUnit[Z_AXIS] + 0.5f);
    deltaDiagonalStepsSquaredA.l = static_cast<uint32_t>((EEPROM::deltaDiagonalCorrectionA() + EEPROM::deltaDiagonalRodLength())*axisStepsPerUnit[Z_AXIS]);
    deltaDiagonalStepsSquaredB.l = static_cast<uint32_t>((EEPROM::deltaDiagonalCorrectionB() + EEPROM::deltaDiagonalRodLength())*axisStepsPerUnit[Z_AXIS]);
    deltaDiagonalStepsSquaredC.l = static_cast<uint32_t>((EEPROM::deltaDiagonalCorrectionC() + EEPROM::deltaDiagonalRodLength())*axisStepsPerUnit[Z_AXIS]);
    if(deltaDiagonalStepsSquaredA.l > 65534 || 2 * radius0*axisStepsPerUnit[Z_AXIS] > 65534)
    {
    ...
    deltaFloorSafetyMarginSteps = DELTA_FLOOR_SAFETY_MARGIN_MM * axisStepsPerUnit[Z_AXIS];
#elif DRIVE_SYSTEM == TUGA
    deltaDiagonalStepsSquared.l = uint32_t(EEPROM::deltaDiagonalRodLength() * axisStepsPerUnit[X_AXIS]);
    ...
    deltaBPosXSteps = static_cast<int32_t>(EEPROM::deltaDiagonalRodLength() * axisStepsPerUnit[X_AXIS]);
    xMaxSteps = static_cast<int32_t>(axisStepsPerUnit[X_AXIS] * (xMin + xLength));
    yMaxSteps = static_cast<int32_t>(axisStepsPerUnit[Y_AXIS] * yLength);
    zMaxSteps = static_cast<int32_t>(axisStepsPerUnit[Z_AXIS] * (zMin + zLength));
    xMinSteps = static_cast<int32_t>(axisStepsPerUnit[X_AXIS] * xMin);
    yMinSteps = 0;
    zMinSteps = static_cast<int32_t>(axisStepsPerUnit[Z_AXIS] * zMin);
#else
    xMaxSteps = static_cast<int32_t>(axisStepsPerUnit[X_AXIS] * (xMin + xLength));
    yMaxSteps = static_cast<int32_t>(axisStepsPerUnit[Y_AXIS] * (yMin + yLength));
    zMaxSteps = static_cast<int32_t>(axisStepsPerUnit[Z_AXIS] * (zMin + zLength));
    xMinSteps = static_cast<int32_t>(axisStepsPerUnit[X_AXIS] * xMin);
    yMinSteps = static_cast<int32_t>(axisStepsPerUnit[Y_AXIS] * yMin);
    zMinSteps = static_cast<int32_t>(axisStepsPerUnit[Z_AXIS] * zMin);
    ...
    {
        invaxisStepsPerUnit[i] = 1.0f/axisStepsPerUnit[i];
#ifdef RAMP_ACCELERATION
        /** Acceleration in steps/s^3 in printing mode.*/
        maxPrintAccelerationStepsPerSquareSecond[i] = maxAccelerationMMPerSquareSecond[i] * axisStepsPerUnit[i];
        /** Acceleration in steps/s^2 in movement mode.*/
        maxTravelAccelerationStepsPerSquareSecond[i] = maxTravelAccelerationMMPerSquareSecond[i] * axisStepsPerUnit[i];
#endif
    }
    float accel = RMath::max(maxAccelerationMMPerSquareSecond[X_AXIS], maxTravelAccelerationMMPerSquareSecond[X_AXIS]);
    minimumSpeed = accel * sqrt(2.0f / (axisStepsPerUnit[X_AXIS]*accel));
    accel = RMath::max(maxAccelerationMMPerSquareSecond[Z_AXIS], maxTravelAccelerationMMPerSquareSecond[Z_AXIS]);
    minimumZSpeed = accel * sqrt(2.0f / (axisStepsPerUnit[Z_AXIS] * accel));
...
uint8_t Printer::moveTo(float x,float y,float z,float e,float f)
{
    if(x != IGNORE_COORDINATE)
        destinationSteps[X_AXIS] = (x + Printer::offsetX) * axisStepsPerUnit[X_AXIS];
    if(y != IGNORE_COORDINATE)
        destinationSteps[Y_AXIS] = (y + Printer::offsetY) * axisStepsPerUnit[Y_AXIS];
    if(z != IGNORE_COORDINATE)
        destinationSteps[Z_AXIS] = (z + Printer::offsetZ) * axisStepsPerUnit[Z_AXIS];
    if(e != IGNORE_COORDINATE)
        destinationSteps[E_AXIS] = e * axisStepsPerUnit[E_AXIS];
...
uint8_t Printer::moveToReal(float x, float y, float z, float e, float f,bool pathOptimize)
{
    ...
    destinationSteps[X_AXIS] = static_cast<int32_t>(floor(x * axisStepsPerUnit[X_AXIS] + 0.5f));
    destinationSteps[Y_AXIS] = static_cast<int32_t>(floor(y * axisStepsPerUnit[Y_AXIS] + 0.5f));
    destinationSteps[Z_AXIS] = static_cast<int32_t>(floor(z * axisStepsPerUnit[Z_AXIS] + 0.5f));
...
{
    currentPosition[X_AXIS] = static_cast<float>(currentPositionSteps[X_AXIS]) * invaxisStepsPerUnit[X_AXIS];
    currentPosition[Y_AXIS] = static_cast<float>(currentPositionSteps[Y_AXIS]) * invaxisStepsPerUnit[Y_AXIS];
#if NONLINEAR_SYSTEM	
    currentPosition[Z_AXIS] = static_cast<float>(currentPositionSteps[Z_AXIS]) * invaxisStepsPerUnit[Z_AXIS];
#else
    currentPosition[Z_AXIS] = static_cast<float>(currentPositionSteps[Z_AXIS] - zCorrectionStepsIncluded) * invaxisStepsPerUnit[Z_AXIS];
#endif	
...
uint8_t Printer::setDestinationStepsFromGCode(GCode *com)
{
    ...
    if(com->hasNoXYZ() && com->hasE() && isAutoretract()) { // convert into auto retract
        if(relativeCoordinateMode || relativeExtruderCoordinateMode) {
            ...
            p = convertToMM(com->E * axisStepsPerUnit[E_AXIS]); // current position
    ...
    {
        p = convertToMM(com->E * axisStepsPerUnit[E_AXIS]);
        ...
        else
        {
            if(
                ...
                (Extruder::current->tempControl.currentTemperatureC < MIN_EXTRUDER_TEMP  && !Printer::isColdExtrusionAllowed()) ||
#endif
                fabs(p - currentPositionSteps[E_AXIS]) * extrusionFactor > EXTRUDE_MAXLENGTH * axisStepsPerUnit[E_AXIS])
...
void Printer::MemoryPosition()
{
    ...
    memoryE = currentPositionSteps[E_AXIS] * invaxisStepsPerUnit[E_AXIS];
    memoryF = feedrate;
}
...
void Printer::deltaMoveToTopEndstops(float feedrate)
{
    ...
    PrintLine::moveRelativeDistanceInSteps(0, 0, (zMaxSteps + EEPROM::deltaDiagonalRodLength()*axisStepsPerUnit[Z_AXIS]) * 1.5, 0, feedrate, true, true);
    offsetX = offsetY = offsetZ = 0;
    setHoming(false);
}
void Printer::MeasureStepToEndstop(float feedrate) 
{
    for (fast8_t i = 0; i < 3; i++)
        Printer::currentPositionSteps[i] = 0;
	Printer::stepsRemainingAtXHit = -1;
    ...
    Printer::stepsRemainingAtZHit = -1;
    setHoming(true);
    PrintLine::moveRelativeDistanceInSteps(0, 0, (zMaxSteps + EEPROM::deltaDiagonalRodLength()*axisStepsPerUnit[Z_AXIS]) * 3, 0, feedrate, true, true);
    offsetX = offsetY = offsetZ = 0;
    setHoming(false);
}
...
void Printer::homeZAxis() // Delta z homing
{
	...
	if (Endstops::xMax() && Endstops::yMax() && Endstops::zMax()) {
		// Back off for retest
		PrintLine::moveRelativeDistanceInSteps(0, 0, axisStepsPerUnit[Z_AXIS] * -ENDSTOP_Z_BACK_MOVE, 0, Printer::homingFeedrate[Z_AXIS] / ENDSTOP_X_RETEST_REDUCTION_FACTOR, true, true);
		//Endstops::report();
		...
		if (!(Endstops::xMax() || Endstops::yMax() || Endstops::zMax())) {
			...
			if (Endstops::xMax() && Endstops::yMax() && Endstops::zMax()) {
				...
				if(ENDSTOP_Z_BACK_ON_HOME > 0) {
					PrintLine::moveRelativeDistanceInSteps(0, 0, axisStepsPerUnit[Z_AXIS] * -ENDSTOP_Z_BACK_ON_HOME * Z_HOME_DIR,0,homingFeedrate[Z_AXIS], true, true);
					//Endstops::report();
    ...
    currentPositionSteps[Y_AXIS] = 0;
    currentPositionSteps[Z_AXIS] = zMaxSteps - zBedOffset * axisStepsPerUnit[Z_AXIS]; // Extruder is now exactly in the delta center
...
#if defined(ENDSTOP_Z_BACK_ON_HOME)
    if(ENDSTOP_Z_BACK_ON_HOME > 0)
        maxDeltaPositionSteps += axisStepsPerUnit[Z_AXIS] * ENDSTOP_Z_BACK_ON_HOME;
#endif
...
void Printer::homeXAxis()
{
            ...
            (MAX_HARDWARE_ENDSTOP_X && X_MAX_PIN > -1 && X_HOME_DIR == 1 && MAX_HARDWARE_ENDSTOP_Y && Y_MAX_PIN > -1 && Y_HOME_DIR == 1))
    {
        ...
        //PrintLine::moveRelativeDistanceInSteps(axisStepsPerUnit[X_AXIS]*-ENDSTOP_X_BACK_MOVE * X_HOME_DIR,axisStepsPerUnit[Y_AXIS]*-ENDSTOP_X_BACK_MOVE * Y_HOME_DIR,0,0,homingFeedrate[X_AXIS]/ENDSTOP_X_RETEST_REDUCTION_FACTOR,true,false);
        // PrintLine::moveRelativeDistanceInSteps(axisStepsPerUnit[X_AXIS]*2*ENDSTOP_X_BACK_MOVE * X_HOME_DIR,axisStepsPerUnit[Y_AXIS]*2*ENDSTOP_X_BACK_MOVE * Y_HOME_DIR,0,0,homingFeedrate[X_AXIS]/ENDSTOP_X_RETEST_REDUCTION_FACTOR,true,true);
        PrintLine::moveRelativeDistanceInSteps(axisStepsPerUnit[X_AXIS]*-ENDSTOP_X_BACK_MOVE * X_HOME_DIR,0,0,0,homingFeedrate[X_AXIS]/ENDSTOP_X_RETEST_REDUCTION_FACTOR,true,false);
        PrintLine::moveRelativeDistanceInSteps(axisStepsPerUnit[X_AXIS]*2*ENDSTOP_X_BACK_MOVE * X_HOME_DIR,0,0,0,homingFeedrate[X_AXIS]/ENDSTOP_X_RETEST_REDUCTION_FACTOR,true,true);
        setHoming(false);
#if defined(ENDSTOP_X_BACK_ON_HOME)
        if(ENDSTOP_X_BACK_ON_HOME > 0)
            PrintLine::moveRelativeDistanceInSteps(axisStepsPerUnit[X_AXIS]*-ENDSTOP_X_BACK_ON_HOME * X_HOME_DIR,0,0,0,homingFeedrate[X_AXIS],true,false);
        // PrintLine::moveRelativeDistanceInSteps(axisStepsPerUnit[X_AXIS]*-ENDSTOP_X_BACK_ON_HOME * X_HOME_DIR,axisStepsPerUnit[Y_AXIS]*-ENDSTOP_Y_BACK_ON_HOME * Y_HOME_DIR,0,0,homingFeedrate[X_AXIS],true,false);
#endif
        currentPositionSteps[X_AXIS] = (X_HOME_DIR == -1) ? xMinSteps-offX : xMaxSteps+offX;
        currentPositionSteps[Y_AXIS] = 0; //(Y_HOME_DIR == -1) ? yMinSteps-offY : yMaxSteps+offY;
        coordinateOffset[X_AXIS] = 0;
...
{	
    ...
    PrintLine::moveRelativeDistanceInSteps(-2 * steps, 0, 0, 0, homingFeedrate[X_AXIS], true, true);
    setHoming(false);
	PrintLine::moveRelativeDistanceInSteps(axisStepsPerUnit[X_AXIS] * ENDSTOP_X_BACK_MOVE,0,0,0,homingFeedrate[X_AXIS] / ENDSTOP_X_RETEST_REDUCTION_FACTOR, true, false);
    setHoming(true);	
    PrintLine::moveRelativeDistanceInSteps(-axisStepsPerUnit[X_AXIS] * 2 * ENDSTOP_X_BACK_MOVE,0,0,0,homingFeedrate[X_AXIS] / ENDSTOP_X_RETEST_REDUCTION_FACTOR, true, true);
    setHoming(false);
#if defined(ENDSTOP_X_BACK_ON_HOME)
    if(ENDSTOP_X_BACK_ON_HOME > 0)
        PrintLine::moveRelativeDistanceInSteps(axisStepsPerUnit[X_AXIS] * ENDSTOP_X_BACK_ON_HOME,0,0,0,homingFeedrate[X_AXIS], true, false);
#endif
...
#if defined(ENDSTOP_X_BACK_ON_HOME)
        if(ENDSTOP_X_BACK_ON_HOME > 0)
            PrintLine::moveRelativeDistanceInSteps(axisStepsPerUnit[X_AXIS] * -ENDSTOP_X_BACK_ON_HOME * X_HOME_DIR,0,0,0,homingFeedrate[X_AXIS], true, true);
#endif
        currentPositionSteps[X_AXIS] = (X_HOME_DIR == -1) ? xMinSteps - offX : xMaxSteps + offX;
#if NONLINEAR_SYSTEM
		transformCartesianStepsToDeltaSteps(currentPositionSteps, currentNonlinearPositionSteps);
...
void Printer::homeYAxis()
{
    ...
    if ((MIN_HARDWARE_ENDSTOP_Y && Y_MIN_PIN > -1 && Y_HOME_DIR == -1) || (MAX_HARDWARE_ENDSTOP_Y && Y_MAX_PIN > -1 && Y_HOME_DIR == 1))
    {
		...
		transformCartesianStepsToDeltaSteps(currentPositionSteps, currentNonlinearPositionSteps);
#endif
        PrintLine::moveRelativeDistanceInSteps(0,axisStepsPerUnit[Y_AXIS] * -ENDSTOP_Y_BACK_MOVE * Y_HOME_DIR,0,0,homingFeedrate[Y_AXIS] / ENDSTOP_X_RETEST_REDUCTION_FACTOR,true,false);
        PrintLine::moveRelativeDistanceInSteps(0,axisStepsPerUnit[Y_AXIS] * 2 * ENDSTOP_Y_BACK_MOVE * Y_HOME_DIR,0,0,homingFeedrate[Y_AXIS] / ENDSTOP_X_RETEST_REDUCTION_FACTOR,true,true);
        setHoming(false);
#if defined(ENDSTOP_Y_BACK_ON_HOME)
        if(ENDSTOP_Y_BACK_ON_HOME > 0)
            PrintLine::moveRelativeDistanceInSteps(0,axisStepsPerUnit[Y_AXIS] * -ENDSTOP_Y_BACK_ON_HOME * Y_HOME_DIR,0,0,homingFeedrate[Y_AXIS],true,false);
#endif
        currentPositionSteps[Y_AXIS] = (Y_HOME_DIR == -1) ? yMinSteps - offY : yMaxSteps + offY;
#if NONLINEAR_SYSTEM
		transformCartesianStepsToDeltaSteps(currentPositionSteps, currentNonlinearPositionSteps);
...
//#endif
	...
	//PrintLine::moveRelativeDistanceInSteps(0,0,axisStepsPerUnit[Z_AXIS]*Z_UP_AFTER_HOME * Z_HOME_DIR,0,homingFeedrate[Z_AXIS],true,false);
	if(zaxis)
...
{
#if DRIVE_SYSTEM == DELTA
    step = (2 * Printer::axisStepsPerUnit[Z_AXIS] * DISTORTION_CORRECTION_R) / (DISTORTION_CORRECTION_POINTS - 1.0f);
    radiusCorrectionSteps = DISTORTION_CORRECTION_R * Printer::axisStepsPerUnit[Z_AXIS];
#else
	xCorrectionSteps = (DISTORTION_XMAX - DISTORTION_XMIN) * Printer::axisStepsPerUnit[X_AXIS] / (DISTORTION_CORRECTION_POINTS - 1);
	xOffsetSteps = DISTORTION_XMIN * Printer::axisStepsPerUnit[X_AXIS];
	yCorrectionSteps = (DISTORTION_YMAX - DISTORTION_YMIN) * Printer::axisStepsPerUnit[Y_AXIS] / (DISTORTION_CORRECTION_POINTS - 1);
	yOffsetSteps = DISTORTION_YMIN * Printer::axisStepsPerUnit[Y_AXIS];

#endif	
    zStart = DISTORTION_START_DEGRADE * Printer::axisStepsPerUnit[Z_AXIS];
    zEnd = DISTORTION_END_HEIGHT * Printer::axisStepsPerUnit[Z_AXIS];
}
...
bool Distortion::measure(void)
{
	...
	int32_t zCorrection = 0;
#if Z_HOME_DIR < 0
	zCorrection += Printer::zBedOffset * Printer::axisStepsPerUnit[Z_AXIS];
#endif		
    ...
    for (iy = DISTORTION_CORRECTION_POINTS - 1; iy >= 0; iy--)
        for (ix = 0; ix < DISTORTION_CORRECTION_POINTS; ix++)
        {
...
#endif
#if DRIVE_SYSTEM == DELTA
            float mtx = Printer::invaxisStepsPerUnit[X_AXIS] * (ix * step - radiusCorrectionSteps);
            float mty = Printer::invaxisStepsPerUnit[Y_AXIS] * (iy * step - radiusCorrectionSteps);
#else
			float mtx = Printer::invaxisStepsPerUnit[X_AXIS] * (ix * xCorrectionSteps + xOffsetSteps);
			float mty = Printer::invaxisStepsPerUnit[Y_AXIS] * (iy * yCorrectionSteps + yOffsetSteps);
#endif			
            ...
            setMatrix(floor(0.5f + Printer::axisStepsPerUnit[Z_AXIS] * (z -zp)) + zCorrection,
            matrixIndex(ix,iy));
        }
	Printer::finishProbing();
#if (DRIVE_SYSTEM == DELTA) && DISTORTION_EXTRAPOLATE_CORNERS
    ...
    Printer::zLength -= sum * Printer::invaxisStepsPerUnit[Z_AXIS];
	*/
#if EEPROM_MODE
...
void Distortion::set(float x,float y,float z) {
#if DRIVE_SYSTEM == DELTA
	int ix = (x * Printer::axisStepsPerUnit[Z_AXIS] + radiusCorrectionSteps + step / 2) / step;
	int iy = (y * Printer::axisStepsPerUnit[Z_AXIS] + radiusCorrectionSteps + step / 2) / step;
#else
	int ix = (x * Printer::axisStepsPerUnit[X_AXIS] - xOffsetSteps + xCorrectionSteps / 2) / xCorrectionSteps;
	int iy = (y * Printer::axisStepsPerUnit[Y_AXIS] - yOffsetSteps + yCorrectionSteps / 2) / yCorrectionSteps;
#endif	
	if(ix < 0) ix = 0;
	if(iy < 0) iy = 0;
	...
	setMatrix(z * Printer::axisStepsPerUnit[Z_AXIS],idx);
}
...
#if DRIVE_SYSTEM == DELTA
			float x = (-radiusCorrectionSteps + ix * step) * Printer::invaxisStepsPerUnit[Z_AXIS];
			float y = (-radiusCorrectionSteps + iy * step) * Printer::invaxisStepsPerUnit[Z_AXIS];
#else
			float x = (xOffsetSteps + ix * xCorrectionSteps) * Printer::invaxisStepsPerUnit[X_AXIS];
			float y = (yOffsetSteps + iy * yCorrectionSteps) * Printer::invaxisStepsPerUnit[Y_AXIS];
#endif			
			int32_t idx = matrixIndex(ix, iy);
			float z = getMatrix(idx) * Printer::invaxisStepsPerUnit[Z_AXIS];
...
void Printer::showJSONStatus(int type) {
    ...
    for (int i = 0; i < NUM_EXTRUDER; i++) {
        ...
        Com::print(extruder[i].extrudePosition / extruder[i].StepsPerUnit);
    ...
    switch (type) {
        ...
        case 5:
            ...
            Com::printF(PSTR("AVR"));
#endif
            Com::printF(PSTR("\",\"firmwareName\":\"Wangsamas\",\"firmwareVersion\":\""));
            Com::printF(PSTR(WANGSAMAS_VERSION));
