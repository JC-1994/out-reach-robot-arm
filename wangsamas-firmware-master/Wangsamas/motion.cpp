...
void PrintLine::rotateInSteps(int32_t x, int32_t y, float feedrate, bool waitEnd, bool checkEndstop,bool pathOptimize)	// Kusuma SCARA
{
    ...
    Printer::destinationSteps[X_AXIS] = Printer::currentNonlinearPositionSteps[X_AXIS] + x;		
    Printer::destinationSteps[Y_AXIS] = Printer::currentNonlinearPositionSteps[Y_AXIS] + y;		
    Printer::destinationSteps[Z_AXIS] = Printer::currentNonlinearPositionSteps[Z_AXIS] ;
    Printer::destinationSteps[E_AXIS] = Printer::currentNonlinearPositionSteps[E_AXIS] ;
    ...
    if (!queueRotation(checkEndstop, pathOptimize, false))
    {
...
void PrintLine::rotateInStepsNoCheck(int32_t x, int32_t y, float feedrate, bool waitEnd, bool checkEndstop,bool pathOptimize)	// Kusuma SCARA
{
	float savedFeedrate = Printer::feedrate;
    Printer::destinationSteps[X_AXIS] = Printer::currentNonlinearPositionSteps[X_AXIS] + x;		
    Printer::destinationSteps[Y_AXIS] = Printer::currentNonlinearPositionSteps[Y_AXIS] + y;		
...
{
    Printer::lastCmdPos[X_AXIS] += x * Printer::invaxisStepsPerUnit[X_AXIS];
    Printer::lastCmdPos[Y_AXIS] += y * Printer::invaxisStepsPerUnit[Y_AXIS];
    Printer::lastCmdPos[Z_AXIS] += z * Printer::invaxisStepsPerUnit[Z_AXIS];
    if(!Printer::isPositionAllowed( Printer::lastCmdPos[X_AXIS], Printer::lastCmdPos[Y_AXIS], Printer::lastCmdPos[Z_AXIS]))
    {
        return; // ignore move
    }
    ...
    Printer::moveToReal(Printer::lastCmdPos[X_AXIS], Printer::lastCmdPos[Y_AXIS], Printer::lastCmdPos[Z_AXIS],
                        (Printer::currentPositionSteps[E_AXIS] + e) * Printer::invaxisStepsPerUnit[E_AXIS],feedrate,pathOptimize);
    Printer::updateCurrentPosition();
    if(waitEnd)
        Commands::waitUntilEndOfAllMoves();
    previousMillisCmd = HAL::timeInMilliseconds();
...
#if DEBUG_DISTORTION	
	Com::printF(PSTR("zCorr:"),Printer::zCorrectionStepsIncluded*Printer::invaxisStepsPerUnit[Z_AXIS],3);
	Com::printF(PSTR(" atX:"),Printer::destinationSteps[0]*Printer::invaxisStepsPerUnit[X_AXIS]);	
	Com::printFLN(PSTR(" atY:"),Printer::destinationSteps[1]*Printer::invaxisStepsPerUnit[Y_AXIS]);
#endif	
    ...
    float axisDistanceInUnit[E_AXIS_ARRAY]; // Axis movement in mm
    ...
    for(uint8_t axis = 0; axis < 4; axis++)
    {
	    ...
	    if(axis == E_AXIS)
	    {
		    if(Printer::mode == PRINTER_MODE_FFF)
		    {
			    ...
			    Printer::filamentPrinted += p->delta[E_AXIS] * Printer::invaxisStepsPerUnit[axis];
		    }
	    ...
	    axisDistanceInUnit[axis] = p->delta[axis] * Printer::invaxisStepsPerUnit[axis];
    ...
    if((p->isXYZMove()) && ((p->dir & XYZ_DIRPOS)^(Printer::backlashDir & XYZ_DIRPOS)) & (Printer::backlashDir >> 3))   // We need to compensate backlash, add a move
    {
	    ...
	    {
		    float f = back_diff[i]*Printer::axisStepsPerUnit[i];
    ...
    if(p->isXYZMove())
    {
	    xydist2 = axisDistanceInUnit[X_AXIS] * axisDistanceInUnit[X_AXIS] + axisDistanceInUnit[Y_AXIS] * axisDistanceInUnit[Y_AXIS];
	    if(p->isZMove())
	    p->distance = RMath::max((float)sqrt(xydist2 + axisDistanceInUnit[Z_AXIS] * axisDistanceInUnit[Z_AXIS]),fabs(axisDistanceInUnit[E_AXIS]));
	    else
	    p->distance = RMath::max((float)sqrt(xydist2),fabs(axisDistanceInUnit[E_AXIS]));
    }
    else
    p->distance = fabs(axisDistanceInUnit[E_AXIS]);
    p->calculateMove(axisDistanceInUnit,pathOptimize,p->primaryAxis);
...
void PrintLine::queueCartesianMove(uint8_t check_endstops, uint8_t pathOptimize)
{
	...
	if(Printer::distortion.isEnabled() && Printer::destinationSteps[Z_AXIS] < Printer::distortion.zMaxSteps() && Printer::isZProbingActive() == false && !Printer::isHoming()) {
		...
		float dx = Printer::invaxisStepsPerUnit[X_AXIS] * deltas[X_AXIS];
		float dy = Printer::invaxisStepsPerUnit[Y_AXIS] * deltas[Y_AXIS];
...
void PrintLine::calculateMove(float axisDistanceInUnit[], uint8_t pathOptimize,fast8_t drivingAxis)
{
#if NONLINEAR_SYSTEM
    ...
    if(isXMove())
    {
        axisInterval[X_AXIS] = axisDistanceInUnit[X_AXIS] * toTicks / (Printer::maxFeedrate[X_AXIS]); // mm*ticks/s/(mm/s*steps) = ticks/step
    ...
    if(isYMove())
    {
        axisInterval[Y_AXIS] = axisDistanceInUnit[Y_AXIS] * toTicks / Printer::maxFeedrate[Y_AXIS];
#if !NONLINEAR_SYSTEM || defined(FAST_COREXYZ)
    ...
    {
        axisInterval[Z_AXIS] = axisDistanceInUnit[Z_AXIS] * toTicks / Printer::maxFeedrate[Z_AXIS]; // must prevent overflow!
#if !NONLINEAR_SYSTEM || defined(FAST_COREXYZ)
    ...
    if(isEMove())
    {
        axisInterval[E_AXIS] = axisDistanceInUnit[E_AXIS] * toTicks / Printer::maxFeedrate[E_AXIS];
        limitInterval = RMath::max(axisInterval[E_AXIS], limitInterval);
    }
    else axisInterval[E_AXIS] = 0;
#if DRIVE_SYSTEM == DELTA
    if(axisDistanceInUnit[VIRTUAL_AXIS] >= 0) {// only for deltas all speeds in all directions have same limit
        axisInterval[VIRTUAL_AXIS] = axisDistanceInUnit[VIRTUAL_AXIS] * toTicks / (Printer::maxFeedrate[Z_AXIS]);
    ...
    if(isXMove())
    {
        ...
        speedX = axisDistanceInUnit[X_AXIS] * inverseTimeS;
    ...
    if(isYMove())
    {
        ...
        speedY = axisDistanceInUnit[Y_AXIS] * inverseTimeS;
    ...
    if(isZMove())
    {
        ...
        speedZ = axisDistanceInUnit[Z_AXIS] * inverseTimeS;
    ...
    if(isEMove())
    {
        ...
        speedE = axisDistanceInUnit[E_AXIS] * inverseTimeS;
    ...
    {
        float advlin = fabs(speedE) * Extruder::current->advanceL * 0.001 * Printer::axisStepsPerUnit[E_AXIS];
...
uint8_t transformCartesianStepsToDeltaSteps(int32_t cartesianPosSteps[], int32_t scaraPosSteps[])
{
	...
	float tempb = cartesianPosSteps[Y_AXIS]*Printer::invaxisStepsPerUnit[Y_AXIS];
	float tempx, tempy;
	TransformToScaraCenter(tempa,tempb,tempx,tempy);
	ScaraInverseKinematics(tempx,tempy,tempa,tempb);
	scaraPosSteps[X_AXIS] = tempa*Printer::axisStepsPerUnit[X_AXIS];
	scaraPosSteps[Y_AXIS] = tempb*Printer::axisStepsPerUnit[Y_AXIS];
	scaraPosSteps[Z_AXIS] = cartesianPosSteps[Z_AXIS];
	scaraPosSteps[E_AXIS] = cartesianPosSteps[E_AXIS];
	return 1;
}

uint8_t transformScaraStepsToCartesianSteps(int32_t scaraPosSteps[], int32_t cartesianPosSteps[])
{
	float tempa = scaraPosSteps[X_AXIS]*Printer::invaxisStepsPerUnit[X_AXIS];
	float tempb = scaraPosSteps[Y_AXIS]*Printer::invaxisStepsPerUnit[Y_AXIS];
	float tempx, tempy;
	ScaraForwardKinematics(tempa,tempb,tempx,tempy);
	TransformToBedCenter(tempx,tempy,tempa,tempb);
	cartesianPosSteps[X_AXIS] = tempa*Printer::axisStepsPerUnit[X_AXIS];
	cartesianPosSteps[Y_AXIS] = tempb*Printer::axisStepsPerUnit[Y_AXIS];
	cartesianPosSteps[Z_AXIS] = scaraPosSteps[Z_AXIS];
...
uint8_t TransformToBedCenter(float tempx1, float tempy1, float &tempx2, float &tempy2)	// Kusuma SCARA
{																		// Kusuma SCARA
	tempx2 = tempx1 - Printer::BedCenterXpos;									// Kusuma SCARA
	tempy2 = tempy1 - Printer::BedCenterYpos;									// Kusuma SCARA
	return 1;															// Kusuma SCARA
...
*/
inline uint16_t PrintLine::calculateRotationSubSegments(uint8_t softEndstop)	// Kusuma SCARA
{
    fast8_t i;
    int32_t delta,diff;
    int32_t destinationDeltaSteps[TOWER_ARRAY], OldPositionSteps[TOWER_ARRAY];
    // Save current position
    for(uint8_t i = 0; i < Z_AXIS_ARRAY; i++)
	{
        destinationDeltaSteps[i] = Printer::currentNonlinearPositionSteps[i];
        OldPositionSteps[i] = Printer::currentNonlinearPositionSteps[i];		
	}
#ifdef DEBUG_STEPCOUNT
    ...
    for (int s = numNonlinearSegments; s > 0; s--)
    {
        ...
        for(i = 0; i < Z_AXIS_ARRAY; i++)
        {
            ...
            diff = Printer::destinationSteps[i] - destinationDeltaSteps[i];
            if(s == 1)
                destinationDeltaSteps[i] += diff;
            else if(s == 2)
                destinationDeltaSteps[i] += (diff >> 1);
            else if(s == 4)
                destinationDeltaSteps[i] += (diff >> 2);
            else if(diff < 0)
                destinationDeltaSteps[i] -= HAL::Div4U2U(-diff, s);
            else
                destinationDeltaSteps[i] += HAL::Div4U2U(diff, s);
        }
            d->dir = 0;
    ...
    for(uint8_t i = 0; i < Z_AXIS_ARRAY; i++)
	{
        Printer::currentNonlinearPositionSteps[i] = OldPositionSteps[i];		
	}
    return maxAxisSteps;
}			// Kusuma SCARA
...
uint8_t PrintLine::calculateDistance(float axisDistanceInUnit[], uint8_t dir, float *distance)
{
    // Calculate distance depending on direction
    if(dir & XYZ_STEP)
    {
        if(dir & XSTEP)
            *distance = axisDistanceInUnit[X_AXIS] * axisDistanceInUnit[X_AXIS];
        else
            *distance = 0;
        if(dir & YSTEP)
            *distance += axisDistanceInUnit[Y_AXIS] * axisDistanceInUnit[Y_AXIS];
        if(dir & ZSTEP)
            *distance += axisDistanceInUnit[Z_AXIS] * axisDistanceInUnit[Z_AXIS];
        *distance = RMath::max((float)sqrt(*distance), axisDistanceInUnit[E_AXIS]);
        return 1;
    }
    else
    {
        if(dir & ESTEP)
        {
            *distance = axisDistanceInUnit[E_AXIS];
            return 1;
        }
        *distance = 0;
        return 0;
...
inline void PrintLine::queueEMove(int32_t extrudeDiff,uint8_t check_endstops,uint8_t pathOptimize)
{
    ...
    PrintLine *p = getNextWriteLine();
    float axisDistanceInUnit[VIRTUAL_AXIS_ARRAY]; // Axis movement in mm
    ...
    axisDistanceInUnit[E_AXIS] = p->distance = p->delta[E_AXIS] * Printer::invaxisStepsPerUnit[E_AXIS];
    axisDistanceInUnit[VIRTUAL_AXIS] = -p->distance;
    p->moveID = lastMoveID++;
...
uint8_t PrintLine::queueRotation(uint8_t check_endstops,uint8_t pathOptimize, uint8_t softEndstop)	// Kusuma SCARA
{
    ...
    float axisDistanceInUnit[VIRTUAL_AXIS_ARRAY]; // Real cartesian axis movement in mm. Virtual axis in 4;
    uint8_t secondSpeed = Printer::fanSpeed;
	for(fast8_t axis = 0; axis < E_AXIS_ARRAY; axis++)
    {
        difference[axis] = Printer::destinationSteps[axis] - Printer::currentNonlinearPositionSteps[axis];
        axisDistanceInUnit[axis] = fabs(difference[axis] * Printer::invaxisStepsPerUnit[axis]);
    }
    ...
    if (!calculateDistance(axisDistanceInUnit, cartesianDir, &cartesianDistance))
    {
    ...
    if(numLines > 1)
    {
        for (fast8_t i = 0; i < Z_AXIS_ARRAY; i++)
            startPosition[i] = Printer::currentNonlinearPositionSteps[i];
    ...
    Printer::currentNonlinearPositionSteps[E_AXIS] = 0;
    ...
    for (int lineNumber = 1; lineNumber <= numLines; lineNumber++)
    {
        ...
        else
        {
            for (fast8_t i = 0; i < E_AXIS_ARRAY; i++)
            {
                ...
                fractionalSteps[i] = Printer::destinationSteps[i] - Printer::currentNonlinearPositionSteps[i];
                axisDistanceInUnit[i] = fabs(fractionalSteps[i] * Printer::invaxisStepsPerUnit[i]);
            }
        ...
        uint16_t maxStepsPerSegment = p->calculateRotationSubSegments(softEndstop);
		...
		if(cartesianDeltaSteps[Y_AXIS] > cartesianDeltaSteps[X_AXIS])
				drivingAxis = Y_AXIS;
        p->primaryAxis = VIRTUAL_AXIS; // Virtual axis will lead Bresenham step either way
            p->stepsRemaining = virtualAxisSteps;
            axisDistanceInUnit[VIRTUAL_AXIS] = p->distance;  //virtual_axis_move * Printer::invaxisStepsPerUnit[Z_AXIS]; // Steps/unit same as all the towers
            // Virtual axis steps per segment
            p->numPrimaryStepPerSegment = maxStepsPerSegment;
		p->calculateMove(axisDistanceInUnit, pathOptimize,drivingAxis);
        for (fast8_t i = 0; i < E_AXIS_ARRAY; i++)
        {
            Printer::currentNonlinearPositionSteps[i] += fractionalSteps[i];
			transformScaraStepsToCartesianSteps(Printer::currentNonlinearPositionSteps, Printer::currentPositionSteps);
        }
    }
...
uint8_t PrintLine::queueNonlinearMove(uint8_t check_endstops,uint8_t pathOptimize, uint8_t softEndstop)
{
    ...
    for(fast8_t axis = 0; axis < E_AXIS_ARRAY; axis++)
    {
        ...
        if(axis == E_AXIS)
        {
            if(Printer::mode == PRINTER_MODE_FFF)
            {
                ...
                axisDistanceInUnit[E_AXIS] = difference[E_AXIS] * Printer::invaxisStepsPerUnit[E_AXIS];
                Printer::filamentPrinted += axisDistanceInUnit[E_AXIS];
                axisDistanceInUnit[E_AXIS] = fabs(axisDistanceInUnit[E_AXIS]);
            }
#if defined(SUPPORT_LASER) && SUPPORT_LASER
            else if(Printer::mode == PRINTER_MODE_LASER)
            {
                secondSpeed = ((axisDistanceInUnit[X_AXIS] != 0 || axisDistanceInUnit[Y_AXIS] != 0) && (LaserDriver::laserOn || axisDistanceInUnit[E_AXIS] != 0) ? LaserDriver::intensity : 0);
                axisDistanceInUnit[E_AXIS] = 0;
            }
#endif
        }
        else
    ...
    for (int lineNumber = 1; lineNumber <= numLines; lineNumber++)
    {
        ...
        else
        {
            ...
            axisDistanceInUnit[VIRTUAL_AXIS] = -p->distance; //p->stepsRemaining * Printer::invaxisStepsPerUnit[Z_AXIS];
			drivingAxis = E_AXIS;
        }
#ifdef DEBUG_SPLIT
...
void PrintLine::arc(float *position, float *target, float *offset, float radius, uint8_t isclockwise)
{
    ...
    float extruder_travel = (Printer::destinationSteps[E_AXIS] - Printer::currentPositionSteps[E_AXIS]) * Printer::invaxisStepsPerUnit[E_AXIS];
    ...
    */
    ...
    arc_target[E_AXIS] = Printer::currentPositionSteps[E_AXIS] * Printer::invaxisStepsPerUnit[E_AXIS];
