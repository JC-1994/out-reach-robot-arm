...
#error You need to define 2 motors for motorized bed correction
    ...
    Printer::currentPositionSteps[Z_AXIS] = h1 * Printer::axisStepsPerUnit[Z_AXIS];
#else
...
bool runBedLeveling(GCode *com) {
    ...
    for(int r = 0; r < BED_LEVELING_REPETITIONS; r++) {
        ...
        float currentZ = plane.z((float)Printer::currentPositionSteps[X_AXIS] * Printer::invaxisStepsPerUnit[X_AXIS],(float)Printer::currentPositionSteps[Y_AXIS] * Printer::invaxisStepsPerUnit[Y_AXIS]);
...
#endif
        Printer::currentPositionSteps[Z_AXIS] = currentZ * Printer::axisStepsPerUnit[Z_AXIS];
...
#endif
    float distance = (float)probeDepth * invaxisStepsPerUnit[Z_AXIS];
...
void Printer::startProbing(bool runScript) {
    ...
    PrintLine::moveRelativeDistanceInSteps((Printer::offsetX - oldOffX) * Printer::axisStepsPerUnit[X_AXIS],
                                           (Printer::offsetY - oldOffY) * Printer::axisStepsPerUnit[Y_AXIS],
...
void Printer::finishProbing() {
    ...
    if(Extruder::current) {
        Printer::offsetX = -Extruder::current->xOffset * Printer::invaxisStepsPerUnit[X_AXIS];
        Printer::offsetY = -Extruder::current->yOffset * Printer::invaxisStepsPerUnit[Y_AXIS];
        Printer::offsetZ = -Extruder::current->zOffset * Printer::invaxisStepsPerUnit[Z_AXIS];
    }
    PrintLine::moveRelativeDistanceInSteps((Printer::offsetX - oldOffX) * Printer::axisStepsPerUnit[X_AXIS],
                                           (Printer::offsetY - oldOffY) * Printer::axisStepsPerUnit[Y_AXIS],
                                           (Printer::offsetZ - oldOffZ) * Printer::axisStepsPerUnit[Z_AXIS], 0, EEPROM::zProbeXYSpeed(), true, ALWAYS_CHECK_ENDSTOPS);
}
...
float Printer::runZProbe(bool first,bool last,uint8_t repeat,bool runStartScript) {
    ...
    int32_t shortMove = static_cast<int32_t>((float)Z_PROBE_SWITCHING_DISTANCE * axisStepsPerUnit[Z_AXIS]); // distance to go up for repeated moves
    ...
    for(int8_t r = 0; r < repeat; r++) {
        ...
        //int32_t offx = axisStepsPerUnit[X_AXIS] * EEPROM::zProbeXOffset();
        //int32_t offy = axisStepsPerUnit[Y_AXIS] * EEPROM::zProbeYOffset();
    ...
    float distance = static_cast<float>(sum) * invaxisStepsPerUnit[Z_AXIS] / static_cast<float>(repeat) + EEPROM::zProbeHeight();
    ...
    if(Printer::distortion.isEnabled()) {
        zCorr = distortion.correct(currentPositionSteps[X_AXIS] + EEPROM::zProbeXOffset() * axisStepsPerUnit[X_AXIS],currentPositionSteps[Y_AXIS]
                                   + EEPROM::zProbeYOffset() * axisStepsPerUnit[Y_AXIS],0) * invaxisStepsPerUnit[Z_AXIS];
        distance += zCorr;
    }
#endif
